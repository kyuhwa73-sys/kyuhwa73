...
class AssemblyClient:
    ...
    def __init__(self, api_key: str = "", timeout: int = 30) -> None:
        ...
        self._session.verify = False  # 기관 네트워크 SSL 프록시 우회
    ...
    def fetch_bills_in_range(
        ...
        """
        지정 기간 내 의원발의 법률안 수집 (페이지별 조기 종료 지원)
        ...
        age : int
            국회 대수
        start_date : str
            발의일 시작 (YYYY-MM-DD 또는 YYYYMMDD)
        end_date : str
            발의일 종료 (YYYY-MM-DD 또는 YYYYMMDD)
        max_bills : int | None
            반환할 최대 법안 수
        verbose : bool
            진행 상황 출력 여부
        on_page : callable | None
            페이지 수집 후 호출되는 콜백 함수
            signature: on_page(page, total_pages, page_bills, matched_so_far)
        """

        def normalize(d: str) -> str:
            return d.replace("-", "").replace("/", "").strip() if d else ""

        norm_start = normalize(start_date)
        norm_end   = normalize(end_date)
        ...
        while True:
            ...
            total_count, bills = self.fetch_bills(
                age=age, page=page, page_size=PAGE_SIZE
            )
            total_pages = max(1, -(-total_count // PAGE_SIZE))  # ceiling div

            if not bills:
                break

            # 이 페이지의 날짜 범위 파악 (빠른 조기 종료 판단용)
            page_dates = [normalize(b.propose_dt) for b in bills if b.propose_dt]
            page_min = min(page_dates) if page_dates else ""
            page_max = max(page_dates) if page_dates else ""

            if verbose:
                print(f"{len(bills)}건 ({page_min} ~ {page_max}), 매칭: {len(filtered)}건")

            # 콜백 호출 (Streamlit 진행 표시용)
            if on_page:
                on_page(page, total_pages, bills, len(filtered))

            # 날짜 필터 적용
            for bill in bills:
                bill_dt = normalize(bill.propose_dt)
                if norm_end and bill_dt > norm_end:
                    continue
                if norm_start and bill_dt < norm_start:
                    continue
                filtered.append(bill)

            # ── 조기 종료 조건 ────────────────────────────────
            # API는 최신 법안 순으로 반환하므로,
            # 이 페이지의 가장 오래된 법안이 시작일보다 이전이면 더 이상 없음
            if norm_start and page_min and page_min < norm_start:
                if verbose:
                    print(f"  조기 종료: 페이지 최소 날짜({page_min}) < 시작일({norm_start})")
                break

            # 최대 건수 도달
            if max_bills and len(filtered) >= max_bills:
                break

            # 마지막 페이지 도달
            if len(bills) < PAGE_SIZE or page >= total_pages:
                break
        ...
        if verbose:
            print(f"  최종 {len(filtered)}건 수집")

        if max_bills:
            filtered = filtered[:max_bills]
    ...
    def fetch_sme_related_bills(
        ...
        """
        중소기업 관련 의원발의 법률안 필터링 수집 (키워드 기반 — 레거시)
        """
        if verbose:
